          ...
          {navigation.map(item => {
            ...
            return (
              <div key={item.name} className='relative group'>
                ...
                >
                  <item.icon
                    className={cn("h-5 w-5 flex-shrink-0 transition-colors", isActive ? "text-red-600" : "")}
                  />
                  {!collapsed && (
                  ...
                  {/* Active indicator */}
                  {isActive && <div className='absolute right-2 w-2 h-2 bg-red-500 rounded-full animate-pulse' />}
                </Link>
                ...
                {collapsed && (
                  <div className='absolute left-full ml-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-50'>
                    ...
                    {item.description && <div className='text-xs text-gray-300 mt-1'>{item.description}</div>}
        ...
        <div className='flex-shrink-0 border-t border-gray-100 p-4'>
          <div className='flex items-center space-x-3 p-3 rounded-xl bg-gray-50'>
            ...
            <div className='flex-1 min-w-0'>
              ...
              <p className='text-xs text-gray-500 truncate'>{userData.client?.name || "Mi Negocio"}</p>
            </div>
          </div>
        </div>
      )}
