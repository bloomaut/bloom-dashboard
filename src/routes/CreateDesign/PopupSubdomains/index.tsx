...
const PopupSubdomains = ({
  onConfirm,
  onCancel,
  setShowConfirmation,
  landingId,
  title,
}: PopupSubdomainsProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (formData.landingEngineId) {
      setLoading(true);
      const data = await update(`subdomains/flake-landing/${formData.landingEngineId}`, { landing_id: landingId }, undefined, ENV.DASHBOARD);
            ...
            <>
              <div className={styles.content}>
                <div className={styles.select_type}>
                  <label className={styles.label}>* El dominio será desasignado de cualquier otro sitio web si se encuentra en uso.</label>
                  <select
